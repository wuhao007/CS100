...
dayToHostPairTuple = access_logs.map(lambda log: ((log.date_time.day, log.host), 1)).reduceByKey(lambda a, b: a + b).map(lambda s: s[0])
...
dayAndHostTuple = access_logs.map(lambda log: ((log.date_time.day, log.host), 1)).reduceByKey(lambda a, b: a + b).map(lambda s: (s[0][0], s[1]))
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda s: (s[0], sum(s[1]) / len(s[1])))
                      .cache())
...
badEndpoints = badRecords.map(lambda s: (s.endpoint, 1))

badUniqueEndpoints = badEndpoints.reduceByKey(lambda a, b: a + b).map(lambda s: s[0])
