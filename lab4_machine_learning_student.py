...
trainingAvgRating = trainingRDD.map(lambda (u, m, r): r).reduce(lambda a, b: a + b) / float(trainingRDD.count())
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (i, n): i not in [x[1] for x in myRatedMovies])
                      .map(lambda (i, n): (myRatedMovies[0][0], i)))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (k, v): (v[0][0], v[1], v[0][1])))
